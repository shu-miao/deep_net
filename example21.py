...
'''
获取参数
为了查看是否存在失效或爆炸的神经元，有时需要仔细检查模型的参数
为了获取这些参数，需要遍历所有的可训练的层，提取它们的参数并放入一个列表中
目前可训练的层类型只有全连接
'''

# 在全连接Layer_Dense类中添加一个方法get_parameters来获取参数
class Layer_Dense:
    def __init__(self,inputs,neurons,
                 weight_regularizer_l1=0, weight_regularizer_l2=0,
                 bias_regularizer_l1=0,bias_regularizer_l2=0):
...
# 相应的在模型Model类中添加get_parameters方法
class Model:
    def __init__(self):
        # 用于存储网络层的列表
        self.layers = []
...
# 需要注意一点，当模型不进行训练，而是将已经训练的参数加载到模型中，这个做法不需要优化器的参与
# 所以需要修改模型中的有关参数优化的逻辑，当不需要进行优化时，所有的有关优化的损失函数不参与
# 修改Model中的finalize方法和set方法
class Model:
    def __init__(self):
        # 用于存储网络层的列表
        self.layers = []
...
# 完成上述修改后，现在可以训练一个模型获取其参数，然后创建一个新模型，将之前获取到的参数设置到新模型中


'''
保存模型
利用pickle模型序列化python对象，将其保存到文件中
'''
...
'''
加载参数
以二进制读取模式打开文件，使用pickle从中读取数据，将参数反序列化回一个列表，然后调用之前的set_parameters方法传入加载的参数
'''
class Model:
    def __init__(self):
        # 用于存储网络层的列表
...
'''
保存模型
保存模型和保存参数是两个不同的方法，二者各有优劣
保存模型不仅包括模型中的参数，还有优化器和模型结构，
如果需要继续训练模型，通常加载完整的模型更为理想

保存模型需要借助copy模块支持
'''
# 在Model类中新增save方法来保存整个模型
class Model:
    def __init__(self):
        # 用于存储网络层的列表
        self.layers = []
...
'''
加载模型
在Model类中创建load方法来加载模型
注意：加载模型最好在模型对象尚未存在之前，也就是说通过调用Model类的方法来加载模型，而不是通过对象调用
'''
class Model:
    def __init__(self):
        # 用于存储网络层的列表
...
'''
查看参数、保存参数、加载参数、保存模型、加载模型都已完成，
目前的模型已经具备了所有的神经网络模型的基本特征
只剩下加载模型进行推理或预测，将模型文件投入使用
'''