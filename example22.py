...
confidences = model.predict(X_test[:5]) # 预测方法输出的是置信度
print(confidences)

# 为了使模型预测的结果更加直观，将置信度转换为类别
# 输出层的激活函数是softmax分类器，所以可以通过输出中的最高值对应的索引来确定模型的预测类别
# 可以直接使用激活函数类中的predictions方法来完成这个功能
predictions = model.output_layer_activation.predictions(confidences) # 使用输出层激活函数中的predictions方法
print(predictions) # 此时输出的是一组索引
print(y_test[:5]) # 对比正确答案

# 为了更加直观，添加一个字典，映射索引和对应类别
...
print(prediction)
'''
预测结果是Sandal，但这不是模型的性能问题，
这涉及最后一个需要讨论的问题，也是实际使用中需要特别注意的问题，
传入的原始图像和训练图像的处理方式不同也会导致预测出错
'''
# 对比传入图像和训练图像
...
axes[1].imshow(image_data.reshape(28, 28), cmap='gray')  # 重新调整形状以适应显示
axes[1].set_title('T-shirt Image')
axes[1].axis('off')
plt.show()
# 传入图像为白色背景，训练图像为黑色背景
# 这是由于二者的灰度转换方式不同
# 直接使用像素数学将传入图像进行灰度转换，使其和训练数据保持一致
image_data = cv2.imread('tshirt.png',cv2.IMREAD_GRAYSCALE) # 读取图片（读取灰度图）
image_data = cv2.resize(image_data,(28,28)) # 调整图片的分辨率，使其和训练数据保持一样
...
'''
完整的衣物分类模型代码将会放在example_final.py中
'''